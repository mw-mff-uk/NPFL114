...
if __name__ == "__main__":
    ...
    # Turn distributions into single labels
    mnist.train.data["labels"] = tf.one_hot(mnist.train.data["labels"], depth=MNIST.LABELS)
    mnist.dev.data["labels"] = tf.one_hot(mnist.dev.data["labels"], depth=MNIST.LABELS)
    mnist.test.data["labels"] = tf.one_hot(mnist.test.data["labels"], depth=MNIST.LABELS)
    ...
    for hidden_layer in args.hidden_layers:
        model.add(tf.keras.layers.Dense(
          ...
          kernel_regularizer=(tf.keras.regularizers.L1L2(l2=args.l2) if bool(args.l2) else None)
        ))
        if (args.dropout):
          model.add(tf.keras.layers.Dropout(args.dropout))
