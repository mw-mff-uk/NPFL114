...
if __name__ == "__main__":
    ...
    def acc(probabilities, labels):
      n = probabilities.shape[0]
      predictions = tf.math.argmax(probabilities, axis=1)
      correct = tf.math.reduce_sum(tf.cast(predictions == labels, tf.int32)) 
      return correct / n 

    dev_size = mnist.dev.data["images"].shape[0]

    with open("mnist_ensemble.out", "w") as out_file:
        for model in range(args.models):
            # TODO: Compute the accuracy on the dev set for
            # the individual `models[model]`.
            individual_accuracy = acc(models[model].predict(mnist.dev.data["images"]), mnist.dev.data["labels"])
            ...
            ensemble_probs = tf.zeros([dev_size, MNIST.LABELS])
            for i in range(0, model + 1):
              ensemble_probs += models[i].predict(mnist.dev.data["images"])

            ensemble_accuracy = acc(ensemble_probs, mnist.dev.data["labels"])
